...
class UniformallNegativeSampler(nn.Module):
    def __init__(self, nloc):
        nn.Module.__init__(self)
        self.n_loc = nloc
    def forward(self, trg_seq, k, **kwargs):
        trg_loc = trg_seq[0][1]
        negsamples = torch.arange(1,self.n_loc)
        negsamples = negsamples[negsamples != trg_loc].unsqueeze(0)
        return negsamples, torch.ones(negsamples.shape, dtype=torch.float32)
    
class UniformNegativeSampler(nn.Module):
    def __init__(self, nloc,user_visited_locs,exclude_visited=False):
        nn.Module.__init__(self)
        self.n_loc = nloc
    ...
    def forward(self, trg_seq, k,user, **kwargs):
        ...
        for check_in in trg_seq:
            if not self.exclude_visited:
                samples = np.random.choice(self.n_loc,k)
            else:
                samples = []
                for _ in range(k):
                    sample = np.random.choice(self.n_loc)
