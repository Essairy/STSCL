...
def collect_fn_quadkey(batch, data_source, sampler, region_processer, loc2quadkey=None, k=5, with_trg_quadkey=True):    
    src, trg = zip(*batch)
    user, loc, time, region, day, hour, session, geo_session = [], [], [], [], [], [], [], [] 
    data_size = []
    trg_ = []
    trg_probs_= []
    for e in src:
        # u_, l_, t_, r_, b_ = zip(*e) # user,loc,time,day,hour,region,session,geo_session,first
        u_,l_,t_,d_,h_,r_,s_,f_ = zip(*e)
        data_size.append(len(u_))
        user.append(torch.tensor(u_))
        loc.append(torch.tensor(l_))
        # time.append(torch.tensor(t_))
        day.append(torch.tensor(d_))
        hour.append(torch.tensor(h_))
        session.append(torch.tensor(s_))
        # geo_session.append(torch.tensor(gs_))
        # (L, LEN_QUADKEY)
        r_ = region_processer.numericalize(list(r_))
        region.append(r_)           ####now
    user_ = pad_sequence(user, batch_first=True)
    loc_ = pad_sequence(loc, batch_first=True)  
    # time_ = pad_sequence(time,batch_first=True)
    day_ = pad_sequence(day,batch_first=True)
    hour_ = pad_sequence(hour,batch_first=True)
    session_ =pad_sequence(session,batch_first=True)
    # geo_session_ = pad_sequence(geo_session,batch_first=True)
    # (T, N, LEN_QUADKEY)
    ...
    if with_trg_quadkey:
        ...
        for i, seq in enumerate(trg):
            ...
            for l in range(trg_seq.size(0)):
                ...
                trg_regs.append(region_processer.numericalize(regs))    # 将栅格码转化为唯一id
        ...
        trg_ = trg_.permute(2, 1, 0).contiguous().view(-1, trg_.size(0))    # 128*109*12 -->1744*128
        trg_nov_ = [[not e[-1] for e in seq] for seq in trg]    # 是否是第一次见
        return user_, loc_, day_, hour_, session_, region_, trg_, batch_trg_regs, trg_nov_, trg_probs_, data_size
    else:
        for i, seq in enumerate(trg):
            pos = torch.tensor([[e[1]] for e in seq])
            neg, probs = sampler(seq, k, user=seq[0][0])
        ...
        return user_, loc_, region_, trg_, trg_nov_, trg_probs_, data_size 
